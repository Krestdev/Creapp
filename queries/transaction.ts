...
export interface TransactionProps extends Omit<Transaction, "id" | "proof" | "from" | "to" | "createdAt" | "status"> {
    proof?: File[];
    from?: source;
    to?: source;
    fromBankId?:number;
...
export interface TransferProps extends Omit<Transaction, "id" | "proof" | "from" | "to" | "createdAt" | "status" | "date"> {
  fromBankId: number;
  toBankId: number;
}
...
export class TransactionQuery {
  ...
  approve = async (data:StatusUpdateProps):Promise<{data: Transaction}> => {
    ...
    let payload:{status: Transaction["status"]; reason?: string; validatorId: number} = {...rest};
    if(reason){
  ...
  createTransaction = async(data:TransferProps):Promise<{data: Transaction}> => {
    const formData = new FormData();
