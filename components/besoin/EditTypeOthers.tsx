...
function EditTypeOthers({
  ...
  // Récupérer les données du besoin existant
  ...
  useEffect(() => {
    if (requestData) {
      form.reset({
        label: requestData.data.label,
        description: requestData.data.description,
        amount: requestData.data.amount,
        quantity: requestData.data.quantity,
        unit: requestData.data.unit,
        benef: requestData.data.benef?.[0] || undefined,
        dueDate: format(new Date(requestData.data.dueDate), "yyyy-MM-dd"),
        priority: requestData.data.priority,
        categoryId: requestData.data.categoryId,
  ...
  const { mutate, isPending } = useMutation({
    mutationFn: async (payload: any) => requestQ.update(requestId, payload),
    onSuccess: () => {
    ...
    <Dialog open={open} onOpenChange={onOpenChange} modal={false}>
      ...
      >
        <DialogHeader variant={"secondary"}>
          <DialogTitle>
            {requestData?.data?.label
              ? `Modifier - ${requestData.data.label}`
              : "Modifier le besoin"}
          </DialogTitle>
          <DialogDescription>
                  ...
                  <FormItem>
                    ...
                    <FormControl>
                      <Combobox
                        items={projects}
                        value={
                          projects.find((p) => p.id === field.value) ?? null
                        }
