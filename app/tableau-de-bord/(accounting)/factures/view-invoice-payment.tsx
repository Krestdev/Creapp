      ...
      <DialogContent>
        ...
        <div className="grid gap-2.5">
          {!invoice.payment || invoice.payment.length === 0 ? (
            <div className="px-3 py-2 rounded-sm text-gray-600 text-sm">
              {"Aucun paiement enregistré sur cette facture."}
            </div>
          ) : (
            invoice.payment.map((item, id) => (
              <div
                key={id}
                className={cn(
                  "px-3 py-2 rounded-sm border flex flex-col gap-0.5",
                  styles(item.status),
                )}
              >
                <span className="text-sm font-bold">
                  {XAF.format(item.price)}
                </span>
