...
const formSchema = z.object({
    reason: z.string().min(4, {message: "Motif trop court"}).max(60, {message: "Trop long"})
})
...
function RejectInvoice({open, openChange, payment, purchases}:Props) {
    ...
    const toReject = useMutation({
        mutationFn: async(reason:string) => paymentQ.rejectInvoice({id: payment.id, reason}),
        onSuccess: ()=>{
        ...
        <DialogContent>
            ...
            <Form {...form}>
                <form onSubmit={form.handleSubmit(onSubmit)} className="flex flex-col gap-3">
                    ...
                    <DialogFooter>
                        <Button type="submit" variant={"destructive"} disabled={toReject.isPending} isLoading={toReject.isPending}>{"Rejeter"}</Button>
                        <Button variant={"outline"} onClick={(e)=>{e.preventDefault(); openChange(false); form.reset({reason: ""})}} disabled={toReject.isPending}>{"Annuler"}</Button>
                    </DialogFooter>
                </form>
            </Form>
        </DialogContent>
